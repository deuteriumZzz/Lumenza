        ...
        <div className="mt-4 flex flex-col gap-4 rounded-md border border-border bg-surface p-3 sm:flex-row sm:p-4">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 text-xs text-muted">
              <span role="status" className={`status-pill ${statusPillClass(entry.status)}`}>{entry.status}</span>
              {entry.mocked ? <span className="status-pill bg-surface-raised">мок</span> : null}
            </div>
            {entry.status === "ok" ? <p className="mt-2 whitespace-pre-wrap text-sm text-ink">{entry.text}</p> : null}
            {entry.status === "error" ? (
