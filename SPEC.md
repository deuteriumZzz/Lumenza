...
### Resumable-стриминг в Chat Mode — ✅ РЕАЛИЗОВАНО (2026-07-31)
Токен-за-токеном генерация вместо ожидания полного ответа, по образцу LibreChat's
resumable SSE streams (см. память проекта — референс-сканирование 2026-07-30).
`POST /api/threads/<id>/messages/stream/` делает тот же синхронный pre-flight, что
и `run_chat()` (роутинг/RAG/модерация/резерв кредитов — вынесены в общие
`_prepare_chat_request`/`_hold_credits_or_reject`, которыми теперь пользуются и
`run_chat()`, и новая функция), и при успехе ставит Celery-задачу
(`providers.tasks.stream_chat_task`), возвращая `generation_id`. Сам вызов
провайдера (`adapter.stream_complete()`, новый метод — OpenAI/Anthropic/Gemini/
NVIDIA стримят по-настоящему через свои SDK, NVIDIA и OpenAI делят один helper
`stream_openai_compatible`; Search/Tavily — не streaming API, использует дефолт
базового класса, отдающий весь текст одним "чанком") выполняется в задаче, которая
пишет каждый дельта-токен в буфер генерации (`providers/streaming.py` — обёртка над
уже существующим Redis-based `CACHES["default"]`, тем же, что у Celery-брокера; в
проекте нет `CHANNEL_LAYERS`/pub-sub, поэтому SSE-эндпоинт не push, а поллит буфer
раз в ~200мс).

**Resumable в буквальном смысле**: буфер хранит накопленный текст целиком, а не
список чанков — переподключение (обрыв сети, `EventSource` сам ретраит; или полная
перезагрузка страницы) просто получает текущий текст целиком первым же событием,
без учёта смещений на сервере. Новое поле `Thread.active_generation_id`
(выставляется до постановки задачи в очередь, а не после — иначе на
`CELERY_TASK_ALWAYS_EAGER` порядок инвертируется, см. комментарий в
`start_chat_stream`) позволяет клиенту при загрузке треда обнаружить незавершённую
генерацию и переподключиться, а не потерять её насовсем.

**Сознательные ограничения v1** (проговорены с пользователем до реализации, при
выборе между "SSE без resume", "полный resumable" и "имитация набора текста на
фронтенде" — выбран полный resumable): доставка поллингом, а не push (нет
`CHANNEL_LAYERS`); падение Celery-воркера посреди генерации ловится только
90-секундным бэкстопом в SSE-view и 2-часовым TTL буфера, отдельного job'а для
сверки зависших списаний нет; Gemini-чанки предполагаются инкрементальными
(соглашение `google-genai` 2.12.1), но не проверены на живом API (в этом
dev-окружении нет `GOOGLE_API_KEY`, та же оговорка, что и у нестримингового
Gemini-адаптера).

**Acceptance**: сообщение появляется по мере генерации, а не одним куском;
переподключение (сеть/перезагрузка страницы) продолжает с того места, где
остановился буфер, а не начинает заново и не теряет ответ; отклонённый запрос
(модерация/баланс/невалидная модель/невалидный workspace) возвращает ту же ошибку
и с тем же статусом, что и обычный чат, не открывая SSE-соединение вообще; отмена
кредитов при полном провале всех запасных вариантов работает идентично
нестриминговому пути. Backend 314→330 тестов зелёных; frontend 172→174.

### Code Interpreter в Studio — ✅ РЕАЛИЗОВАНО (2026-07-31)
Новый режим "Код" в творческой студии (`/studio`) — по образцу LibreChat's
Code Interpreter, но намеренно узкий scope: только Python, только
stdout/stderr/код завершения, без файлов/графиков-артефактов (нет
plotting-библиотеки в зависимостях, и сам исполняющий движок их не
возвращает). Соответствует принципу "curation gate" из стратегии этого
документа — конкретный вертикальный сценарий, а не общая dev-песочница.

**Песочница — самостоятельный хостинг Piston** (`ghcr.io/engineer-man/piston`,
MIT, использует `isolate` для изоляции), новый сервис `piston` в
`docker-compose.yml`, `--privileged` (требование самого Piston, без него
изоляция не работает), volume для установленных языков, без публикации
хостового порта — доступен только `web`/`worker` через сеть compose.
**Публичный emkc.org API отпал сам собой**: стал whitelist-only с
2026-02-15 и явно исключает платные/коммерческие продукты — у Lumenza есть
биллинг, так что доступ туда и не рассматривался всерьёз. Самописный
sandboxing (вместо готового движка) тоже отклонён — означал бы писать
изоляцию с нуля самому, а не полагаться на специализированный,
поддерживаемый механизм. Python не предустановлен — одноразовая команда
`manage.py setup_piston` ставит последнюю версию через `POST
/api/v2/packages` (сравнение версий через кортеж чисел, не лексикографически
— `3.9.4` иначе оказался бы "больше" `3.12.0`). Реально проверено вживую в
этой сессии: privileged-контейнер, cgroup, установка Python 3.12.0 и
исполнение кода — всё действительно работает в текущем окружении (не только
через мок-адаптер).

**Реальный баг, найденный в процессе**: Piston по умолчанию отклоняет
запрос с `run_timeout` больше собственного серверного потолка
(`PISTON_RUN_TIMEOUT`, по умолчанию 3000мс) 400-й ошибкой — адаптер
изначально запрашивал 10000мс, что рушилось на реальном инстансе (но не
ловилось моковым путём, естественно). Исправлено поднятием потолка через
`PISTON_RUN_TIMEOUT`/`PISTON_RUN_CPU_TIME`/`PISTON_RUN_MEMORY_LIMIT` в
`environment:` самого сервиса `piston` в docker-compose, синхронно с
константами `RUN_TIMEOUT_MS`/`RUN_MEMORY_LIMIT_BYTES` в
`code_interpreter/piston_adapter.py`.

**Архитектура — тот же паттерн задачи, что и у media_ops** (документы/фото):
`CodeExecution` (переиспользует `media_ops.models.MediaStatus`, без третьего
переопределения того же five-state enum) → `start_code_execution` (резерв
кредитов, `transaction.atomic()`, постановка в очередь через
`core.enqueue.try_enqueue_or_refund`) → Celery-задача `execute_code`
(модерация `check_prompt(code)` — код может пробрасывать запрещённый текст
через `print()`, тот же принцип, что у модерации текста перед TTS в
media_ops — затем реальный вызов, сверка кредитов) → `create_code_execution`
+ `CodeExecutionDetailView` (poll), через общий `core.responses.locked_or`.
Цена — плоская `$0.001` (сдерживающий фактор от злоупотребления общим
ресурсом, а не реальная passthrough-стоимость: self-hosted Piston ничего не
стоит за запрос).

**Acceptance**: пользователь пишет и запускает Python-скрипт из вкладки
"Код" в Studio, видит stdout/stderr/код завершения после короткого опроса
статуса; запрещённый через модерацию код — кредиты возвращены, статус
`blocked`; ошибка выполнения/сети — кредиты возвращены, статус `error`;
история чужого запуска недоступна (404). Backend 330→341 тестов зелёных;
frontend 174→177.

### Автоматизации: расписания + publish в Telegram — ✅ РЕАЛИЗОВАНО (2026-07-31)
Item 9 из "revised implementation order" — закрывает и последний открытый
пункт Phase 17 ("безопасные внешние действия/audit log", ниже был
заблокирован именно отсутствием исходящих интеграций у агентов). В кодовой
базе не было вообще никакой OAuth/publish-инфраструктуры; список
"Threads/Telegram/WordPress/Drive" оказался из черновика в памяти проекта,
а не из актуального SPEC.md — договорились с пользователем сузить до
**ежедневных расписаний запуска агента + публикации в Telegram**, так как
только у Telegram уже есть инфраструктура в проекте (`TELEGRAM_BOT_TOKEN`,
`media_ops/telegram_notify.py`) — Threads/WordPress/Drive потребовали бы
новых OAuth-приложений и ключей от пользователя.

**Жёсткое ограничение из спеки**: пост из расписания никогда не публикуется
автоматически — только после явного подтверждения, даже для полностью
безнадзорного расписания.

Новое приложение `automations`, три модели: `TelegramChannel` (подключается
через `getChat`, подтверждает только что бот видит chat_id, не права на
постинг — те проверяются только в момент реальной отправки),
`ScheduledAgentRun` (ежедневный запуск в hour:minute UTC — без cron/недельной
периодичности в v1 — `input_payload` валидируется тем же
`agents.services.validate_against_input_schema`, что и обычный запуск),
`PendingAction` (черновик — сама запись это и есть audit log, тот же принцип,
что у `AgentRun`/`CodeExecution`: статус-переходы и таймстемпы, отдельной
таблицы аудита нигде в проекте нет).

**Текст публикации всегда приходит от вызывающей стороны, а не выводится на
бэкенде из `AgentRun.result`** — у трёх существующих агентов разная форма
результата (`research-digest`/`document-summary` имеют `summary`,
`threads-content-day` — нет, только `schedule[].post_text`). Кнопка
"Опубликовать в Telegram" на странице запуска агента подставляет разумный
черновик по slug на фронтенде в редактируемый textarea — редактирование
текста и есть preview, подтверждение отправляет ровно то, что показано. Для
черновиков, созданных по расписанию (пользователя нет рядом, некому
заполнить) — `automations.services.default_publish_text` — тот же
принцип на бэкенде как запасной вариант.

**Планировщик переиспользует единственный существующий периодический
паттерн, не добавляет инфраструктуры**: `billing.tasks.renew_due_subscriptions`
— единственная существующая Celery-Beat задача в проекте (обычная функция,
не `@shared_task`, обёрнута в `shared_task` в конце модуля — тесты вызывают
её напрямую) — просканировала строки с `current_period_end__lte=now`, тот же
паттерн для `ScheduledAgentRun.objects.filter(is_active=True,
next_run_at__lte=now)`. Триггер запускает **уже существующий, немодифицированный**
`agents.services.start_agent_run` — те же кредиты/модерация/фолбэки, что у
любого обычного запуска. Второй проход того же 5-минутного скана (не
изменяя `agents/tasks.py`, т.к. `start_agent_run` ставит Celery-задачу и
сразу возвращается, скан не может синхронно узнать исход) создаёт
`PendingAction`-черновик для завершённых запусков с `publish_channel`, и,
если у пользователя привязан `telegram_id`, шлёт напоминание в личку —
best-effort, не часть audit-трейла (сам `PendingAction` уже сохранён
независимо).

**Отправка строже существующих notify-хелперов**: `media_ops/telegram_notify.py`
глотает `URLError` и не проверяет `{"ok": false}` в ответе Telegram — ок для
best-effort уведомлений, не ок здесь, где "действительно ли пост ушёл" — весь
смысл функции. Новый `automations/telegram_client.py` проверяет `"ok"` и
бросает `TelegramApiError` при неудаче, `send_pending_action`
записывает `status=failed` + `error_message`, а не молча "sent".

**Инфраструктурный инцидент во время этой сессии, не связанный с кодом**:
диск хоста дважды заполнялся под ноль (см. похожий инцидент раньше в этом же
проекте) — из-за повторных одноразовых `node:22-bookworm`-контейнеров для
фронтенд-тестов. Docker daemon дважды подвисал даже после освобождения
места (полный `docker builder prune -af` доходил только до нуля прогресса,
процесс жил, но не продвигался) — помог только полный перезапуск Docker
Desktop, после которого `docker builder prune -af` реально освободил ~14ГБ
build-кэша. Реально проверено вживую: `docker compose up -d piston` +
токен бота (`@LumenzaAI_bot`, подтверждён через `getMe` тем же кодом, что
использует `telegram_client.py`) — тестового chat_id для полной проверки
`send_message`/`getChat` не было (нельзя выдумать), так что это лишь
частичная живая проверка.

**Acceptance**: расписание создаётся/приостанавливается/удаляется из
`/automations`; сработавшее расписание запускает агента тем же движком,
что и ручной запуск; результат с `publish_channel` не публикуется сам —
появляется черновиком, ожидающим подтверждения; подтверждение отправляет
ровно отредактированный текст, отклонение/ошибка отправки видны в статусе;
чужие каналы/расписания/черновики недоступны (404). Backend 341→361
тестов зелёных; frontend 177→182.

### "Мои агенты" — лёгкий конструктор — ✅ РЕАЛИЗОВАНО (2026-07-31)
Item 10, последний пункт "revised implementation order". Исследование
`agents.models`/`services`/`tasks` показало, что отдельная запись
`workflow_steps` (`{"key","label","task"}`) не несёт собственного промпта —
`system_instructions`/`input_schema`/`output_schema` существуют один раз на
весь `Agent` и безусловно общие для всех его шагов. Поэтому единственная
структурно переиспользуемая единица — целый существующий опубликованный
`Agent`, а не отдельный шаг. "Комбинирует 2–3 уже одобренных шага из разных
категорий" реализовано как: пользователь выбирает 2–3 целых агента из
каталога (≥2 разных категорий), бэкенд **склеивает их в одну новую
синтетическую `Agent`-запись**, которую выполняет уже существующий,
совершенно немодифицированный движок (`agents.services.render_step_prompt`/
`agents.tasks.run_agent` не тронуты ни строкой).

`Agent` получил `user` (null для глобального каталога, задан для
пользовательского агента — публичный каталог и `AgentRunCreateView`
фильтруются по `user__isnull=True | user=request.user`, включая уже
существующий похожий unscoped lookup в `automations`-эндпоинте создания
расписания, который тем же патчем закрыт) и `source_agent_slugs` (упорядоченный
список исходных slug — фронтенд по последнему элементу выбирает уже
существующий рендерер результата вместо `params.slug`, у сгенерированного
`custom-*` slug рендерера нет). `category` синтетического агента — категория
первого выбранного источника (не отдельное значение enum "custom" — иначе
`render_step_prompt`'s `user_context.get(agent.category)` молча теряет
сохранённый профиль пользователя). Ключи шагов при склейке переписываются
(`{slug}__{key}`) во избежание коллизии (у каждого агента-источника
последний шаг уже называется `"assemble"`), последний шаг всей цепочки
принудительно получает буквальный ключ `"assemble"`. `input_schema` —
объединение полей источников (dedup по key), `output_schema` — schema
последнего источника. "Удаление" — не `.delete()` (у `AgentRun.agent`
`on_delete=PROTECT`, упадёт с `ProtectedError` при наличии хотя бы одного
запуска), а `PATCH status=archived`.

**Acceptance**: конструктор доступен во вкладке "Мои агенты" на `/agents`;
2–3 агента из ≥2 категорий собираются в одного нового агента с объединённой
формой; запуск проходит тем же движком, каждый следующий агент видит
результаты предыдущих; результат рендерится тем же компонентом, что и его
последний источник-агент; чужой кастомный агент недоступен (404) как для
прямого запуска, так и для планирования через `/automations`; публичный
каталог никогда не показывает чужие кастомные агенты. Backend 361→373
тестов зелёных; frontend 182→186.
...
Chat/Agent/Knowledge-переключатель — см. выше).
Backend 298→308 тестов зелёных; frontend 163→169.

**Slice 2 — ✅ РЕАЛИЗОВАНО (2026-07-31): RAG-вложение в чат и агентов.**
`knowledge.services.search()` подключён к обеим точкам генерации, оба —
необязательное вложение, без биллинга поиска (тот же принцип, что у slice 1),
без пересечения аккаунтов (владение проверяется существующей проверкой внутри
`search()`). Чат: `ChatRequestSerializer.workspace_id` — сквозной проброс до
`providers.services.run_chat(..., workspace_id=)`, найденные чанки один раз
подмешиваются в `system` до входа в цикл fallback-провайдеров
(`knowledge.services.format_matches_for_prompt` — общий форматтер для обеих
точек). Агент: `AgentRun.workspace` — первый FK-уровня run-атрибут (не поле
`input_schema`, чтобы не мешать инфраструктурный выбор с бизнес-полями формы),
владение проверяется один раз при создании run в `start_agent_run()`; сам
`run_agent()` достаёт чанки один раз за run (та же экономия запроса, что уже
была у `UserContext`), запрос строится конкатенацией значений
`input_payload`, а `render_step_prompt()` получил новый параметр
`knowledge_context`, подмешиваемый отдельной секцией перед вводом формы.
Фронтенд: новый `WorkspacePicker` (по образцу `PresetPicker`, без формы
создания — воркспейсы заводятся на `/knowledge`) в композере чата (независимо
от `ChatRoutingSelection` — вложение ортогонально выбору модели/пресета) и на
странице запуска агента (отдельно от полей `input_schema`).
Backend 308→314 тестов зелёных; frontend 169 (без изменения числа — 6 новых
тестов компенсированы отсутствием удалений, плюс 6 существующих ассершенов
обновлены под новый последний позиционный аргумент, как и в slice 3 Presets).

**Slice 3 — ✅ РЕАЛИЗОВАНО (2026-07-31): Chat/Agent/Knowledge-переключатель.**
`ThreadSidebar` (`frontend/src/components/thread-sidebar.tsx`) — единственный
хост навигации для всех workspace-роутов и на web, и в Telegram Mini App
(общий `WorkspaceShell`, без отдельной ветки под Telegram) — получил сегмент
Чат/Агенты/Знания над остальными пунктами (Студия остаётся отдельной строкой:
это собственная концепция Lumenza, не часть abacus.ai-механики). Визуально —
тот же sliding-`layoutId`-pill паттерн, что уже был у Studio (`studio/page.tsx`)
и категорий Agents (`agents/page.tsx`), но написан заново для сайдбара, а не
вынесен в общий примитив (те два использования — локальные фильтры внутри
страницы, а не навигация, трогать их не требовалось). Активный пункт
переключателя определяется через `getWorkspaceSection(pathname)`
(`lib/workspace-sections.ts`) — тот же единый источник истины, что уже
использовали `route-transition.tsx`/`zone.tsx`; локальная проверка
`studioActive` в сайдбаре тоже переведена на него. Пункты — настоящие
`<Link>`, не кнопки с `onClick`-навигацией, поэтому все существующие проверки
на ссылки "Агенты"/"Знания" не потребовали изменений. В свёрнутом (iconrail)
режиме пилюль нет — просто добавлена третья иконка "Чат" в общий ряд, тот же
принцип, что уже был у Студии. Frontend 169→172 тестов (3 новых: Chat-пункт в
свёрнутом/развёрнутом виде + `aria-current` на активном пункте), без
изменений на бэкенде.

**✅ ЗАКРЫТО (2026-07-31)**: безопасные внешние действия/audit log —
см. "Автоматизации: расписания + publish в Telegram" выше. Публикация в
Telegram через `automations.PendingAction` — первая (и пока единственная)
исходящая интеграция агентов, с preview → явное подтверждение → execution и
audit-трейлом (сама запись). Другие внешние действия (публикация на других
платформах, изменение сторонних ресурсов) по-прежнему не реализованы —
этот пункт закрыт применительно к Telegram, не ко всем возможным внешним
действиям в общем.
