...
### Phase 16 — Agent Registry + первая вертикаль «Контент на день»
...
версия, статус публикации и eval-набор. Первый агент собирает тему, аудиторию, тон и
цель, затем выдаёт готовый план Threads на день: ветки, хуки, расписание и варианты.
Публикация в соцсеть не входит в первую вертикаль — сначала результат и подтверждение.

**Acceptance**: пользователь запускает агент из каталога, отвечает только на короткую
форму, видит пошаговый статус, получает структурированный результат; каждый run хранит
версию агента, стоимость, выбранные модели и ошибки; повторный запуск идемпотентен.
