...
- **Routing**: task-based — таблица категорий задач (хук/лонгрид/хэштеги/контент-план/репурпоз/перевод для текста; реализм/иллюстрация/мокап/черновик для картинок) → приоритетный список (провайдер, модель) с fallback при отказе. Заменяет прежнюю плоскую схему "быстро/умно/дёшево" (см. Фазу 10)
...
Вся инфраструктура (биллинг, гейт progression, Celery, фронтенд, бот) полностью работает в реальном (не mock) режиме для ASR/TTS.

**Progression**: новые категории в каталоге разблокировки — `premium` (image, 8 запросов/3 дня), `voice_to_text` (12/3), `document_to_text` (16/4), `text_to_voice` (22/5) — все заперты по умолчанию для FREE.
...
| Speech-to-text (новая) | `voice_to_text` — расшифровка голосовых заметок в текст поста | nvidia/canary-1b-asr, nvidia/parakeet-* (мультиязычные), openai/whisper-large-v3 |
| Text-to-speech (новая) | `text_to_voice` — озвучка поста/подкаста | nvidia/magpie-tts-multilingual, nvidia/studiovoice, resembleai/chatterbox-multilingual-tts |
...
Нерелевантные для SMM-ниши модальности каталога (биология/drug-discovery, промышленная симуляция, авто/робототехника, финансы, климат) — сознательно не рассматриваются, даже в рамках "широкого агрегатора": curation gate отсекает по релевантности аудитории, не только по качеству модели.
...
### Phase 12 — Free/Paid тариф + игровая разблокировка
Free tier открывает категории задач постепенно за usage-based прогресс (счётчик + минимум distinct-days, антифарм). Без питомца/декей-механики — уровни/прогресс-бар, серьёзный тон. Paid tier — мгновенный доступ ко всем категориям.
**Acceptance**: FREE-юзер видит прогресс к следующей разблокировке; попытка доступа к запертой категории не списывает кредиты; PAID обходит гейт полностью.
...
### Per-model выбор с геймификацией (2026-07-20) — ✅ РЕАЛИЗОВАНО
...
**Схема**: новая таблица `progression.ModelUnlockable` (task, provider, model, min_requests,
min_distinct_days, sort_order) — ручной снепшот `TASK_ROUTES` (не live-импорт: `providers`
уже импортирует `progression.services`, циклический импорт невозможен). Позиция 0 каждой
категории (primary) имеет `min_requests=min_distinct_days=0` — бесплатна сразу, как только
открыта сама категория. Остальные позиции — простая линейная лестница (+4 запроса / +1 день
за позицию), не индивидуально подобранные пороги (тот же placeholder-дух, что и у NVIDIA-цен).
Разблокировка моделей идёт через тот же единый общий счётчик (`_usage_stats`), что и
разблокировка категорий — не отдельная система трекинга.

**Выбор модели + fallback**: `run_chat(user, prompt, task, model=None)` — если `model` передан,
проверяется через `get_unlocked_models(user, task)`; если разрешён — переставляется в начало
списка кандидатов, а остальной список остаётся как обычный fallback (если выбранная модель
падает, всё равно есть страховка). Если модель неизвестна в категории или ещё не разблокирована
— `status="model_locked"`, 403, ничего не списывается.

**API**: `POST /api/chat/` принимает опциональное поле `model`; `GET /api/progress/models/<task>/`
отдаёт полный список кандидатов категории (locked/unlocked + прогресс) для рендера пикера.

**Frontend**: `/chat` — под селектором категории новый ряд кнопок "Auto" + все кандидаты
категории (🔒 у запертых, с прогрессом в title). Живая проверка через Playwright (реальный
браузер, не только API/build) подтвердила: task-lock и model-lock рендерятся корректно
одновременно, явный выбор модели проходит весь цикл (отправка → ответ с нужной моделью,
`used_fallback: false`).

**Бот НЕ получил пикер** — сохранён прежний прецедент ("не выставлять тот же выбор в боте, что
и в вебе", уже применённый к image task categories для `/image`): бот по-прежнему использует
дефолтный порядок категории без явного выбора модели. Гейминг всё равно работает бесшовно —
прогресс общий на счётчик, просто в боте нет UI для его использования на уровне модели.
