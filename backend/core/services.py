...
# Сколько комбинаций (provider, model) показывать в таблице ошибок —
# достаточно, чтобы поймать нестабильного провайдера, но не дать одной
# шумной строке затеряться в длинном хвосте разовых сбоев.
TOP_ERROR_LIMIT = 20

# Порог и окно для пометки пользователя, который постоянно натыкается на
# блок-лист модерации — случайные ложные срабатывания бывают, но столько
# за час больше похоже на прощупывание, чем на случайную формулировку.
...
def flag_repeated_moderation_blocks(user) -> None:
    """Вызывается после блокировки модерацией на любой из поверхностей (чат
    или генерация изображений). Создаёт один AnomalyFlag при первом
    пересечении порога в пределах окна, а не по одному на каждую
    последующую блокировку — иначе один недобросовестный пользователь
    завалил бы admin дублирующимися флагами вместо одного сигнала, на
    который можно среагировать."""
    since = timezone.now() - MODERATION_BLOCK_ANOMALY_WINDOW
    recent_blocks = (
        RequestLog.objects.filter(
            user=user, status=RequestLog.Status.BLOCKED, created_at__gte=since
        ).count()
...
def margin_dashboard_data(
    window_days: int = DEFAULT_WINDOW_DAYS,
) -> MarginDashboardData:
    """Агрегирует траты на провайдеров против того, что списано с
    пользователей, по пользователю/дню, плюс самые шумные источники ошибок
    (provider, model) — для admin margin dashboard, оставлено как обычная
    функция, чтобы её можно было тестировать без рендеринга HTML."""
    ...
    def _accumulate(queryset):
        ...
        for row in rows:
            ...
            entry = by_user_day.setdefault(
                key, UserDayRow(user=row["user__username"], day=row["day"])
            )
            entry.cost_usd += row["cost_usd"] or Decimal("0")
            entry.revenue_usd += (
                row["credits_charged"] or Decimal("0")
            ) * credit_usd_value
    ...
    ordered_rows = sorted(
        by_user_day.values(), key=lambda r: (r.day, r.user), reverse=True
    )

    top_errors: list[ErrorSourceRow] = []
    chat_errors = (
        RequestLog.objects.filter(
            created_at__gte=since, status=RequestLog.Status.ERROR
        )
    ...
    for row in image_errors:
        source = (
            "image_blocked"
            if row["status"] == GeneratedImage.Status.BLOCKED
            else "image"
        )
        top_errors.append(
            ErrorSourceRow(
    ...
    return MarginDashboardData(
        ...
        total_revenue_usd=sum(
            (r.revenue_usd for r in ordered_rows), Decimal("0")
        ),
